function [bestset, pindex,pscores, bestcond,bestcut,bestvol] = pprgrow(A,vert,varargin)
...
for ei=1:numel(expands)
    ...
    %fprintf('Called pprgrow on set of size=%i with expand=%i\n', numel(vert), curexpand);
     [curset pindex pscores cond cut vol] = pprgrow_mex(A,vert,curexpand,p.Results.alpha);
%    sumppr = sum(pscores);
%     disp(curset)
%     disp(ocurset)r
    if cond < bestcond
